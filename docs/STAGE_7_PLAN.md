...
an ISO-8601 datetime WITH offset** matching the FULL `z.iso.datetime({offset:true})`
grammar — `date T HH:MM`, with `:SS` and `.fraction` **optional**, then a required
`Z` or `±HH:MM` — whose `timestamptz` cast succeeds (that cast is the
calendar-validity check); and every one of the ten collections present as a JSON
array — rejecting any violation with `23514` `invalidStoreEnvelope`. This
...
- **`npm run test:api` resets the local database first** (`db:reset && vitest`),
  ...
  invocations each perform their own reset and each pass in full (160/160 as of
...
| API-level repository tests vs local URL/key | `npm run test:api` — **160 assertions, real HTTP, self-resetting** |
