...
a required `Z` or `±HH:MM` **within ±15:59** — whose `timestamptz` cast succeeds
...
offsets (±16:00 … ±23:59) that `timestamptz` cannot represent —
`isoDateTime()` is refined to the same range so the two sides accept exactly one
set of timestamps (see DOMAIN_SCHEMA.md, "The durable timestamp contract"). This
...
- **`npm run test:api` resets the local database first** (`db:reset && vitest`),
  ...
  invocations each perform their own reset and each pass in full (173/173 as of
...
| API-level repository tests vs local URL/key | `npm run test:api` — **173 assertions, real HTTP, self-resetting** |
