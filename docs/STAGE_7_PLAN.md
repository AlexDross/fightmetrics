...
an ISO-8601 datetime WITH offset** (matching `z.iso.datetime({offset:true})`)
whose `timestamptz` cast succeeds; and every one of the ten collections present as
a JSON array — rejecting any violation with `23514` `invalidStoreEnvelope`. This
...
- **`npm run test:api` resets the local database first** (`db:reset && vitest`),
  ...
  invocations each perform their own reset and each pass in full (155/155 as of
...
| API-level repository tests vs local URL/key | `npm run test:api` — **155 assertions, real HTTP, self-resetting** |
