...
exactly (no missing, no extra), `meta` exactly `{schemaVersion (number),
migratedAt (string|null)}`, and every one of the ten collections present as a
JSON array — and rejects any violation with `23514` `invalidStoreEnvelope`. This
closes a destructive bypass: the first cut coalesced missing collections to `[]`,
so a payload that fails StoreSchema (e.g. `{meta:{…}}` alone) cleared the whole
workspace and returned `200`. Deep per-row validation stays with the JS adapter
(Gate 6) and the DB's own CHECK/FK/triggers on insert; this gate only refuses a
structurally incomplete envelope before it can be destructive.
...
- **`npm run test:api` resets the local database first** (`db:reset && vitest`),
  ...
  invocations each perform their own reset and each pass in full (147/147 as of
  the cluster-8 corrective, across ten files: export, routing/concurrency, cluster 1–2 RPCs,
...
| API-level repository tests vs local URL/key | `npm run test:api` — **147 assertions, real HTTP, self-resetting** |
