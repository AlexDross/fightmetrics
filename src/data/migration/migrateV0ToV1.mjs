...
// one TrackedPosition. That includes NO BET rows and rows with no betAction at
// all: computeROISummary already counts them, so anything less would silently
// rewrite the historical track record.
//
...
export function migrateV0ToV1(legacy, deps) {
  ...
  const sourcedEntries = [
    ...roi.map((entry) => ({ entry, source: 'roi' })),
    ...upcoming.map((entry) => ({ entry, source: 'upcoming' })),
  ];

  // A save/grade handoff updates roiData.js and upcomingData.js together. If
  // only the first half lands, the same legacy id exists in both files and all
  // deterministic child ids collide. Abort with the source locations instead
  // of leaving the relational validator to report a wall of downstream UUID
  // duplicates.
  const legacyIdSources = new Map();
  for (const { entry, source } of sourcedEntries) {
    const id = String(entry?.id);
    const firstSource = legacyIdSources.get(id);
    if (firstSource) {
      errors.push(
        `legacy entry id ${JSON.stringify(id)} appears more than once ` +
        `(${firstSource} and ${source}); ROI and Upcoming updates must be committed together`
      );
    } else {
      legacyIdSources.set(id, source);
    }
  }

  // Stable ordering drives Bout orientation, so it must not depend on array
  // identity or object iteration. createdAt then id is total when the source
  // ids pass the uniqueness guard above.
  const allEntries = sourcedEntries
    .sort((x, y) =>
  ...
  for (const { entry } of allEntries) {
    ...
    predictionRuns.push({
      ...
      createdAt: entry.createdAt,
      decisionSnapshotId,
      // Current legacy rows carry an eventDate, and targetEventDate equals it wherever
    ...
    // all rows in the Stage 6 capture, but that is a characterisation of that seed,
 ...
 * They were absent from every row in the Stage 6 seed capture, which is why an
...
function buildReviewState(entry, errors, runId) {
  ...
  // Neither field: the normal case for the seed rows and every manual save.
