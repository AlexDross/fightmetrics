...
export const LEGACY_FIELD_MAP = Object.freeze({
  ...
  roiEntry: {
    ...
    // Derived from the TRACKED position's own market, not the assessment's.
    // The two diverge as soon as an ROI price correction appends a new
    // MarketSnapshot and repoints TrackedPosition.marketSnapshotId, which is
    // how a price can be fixed without rewriting the frozen assessment.
    marketOdds: { derived: 'TrackedPosition.marketSnapshotId + corner; equals oddsA/oddsB by trackedSide on 160/160 at migration time' },
...
export const GENERATED_FIELD_SOURCES = Object.freeze({
  ...
  'TrackedPosition.marketSnapshotId': 'the assessment market at migration time (158 linked, 2 null); thereafter the price the position is SCORED at, repointed by an ROI price correction without touching the frozen assessment',
