...
OUTPUT_PATH = ROOT / 'src' / 'rankingsData.js'
...
def write_generated(cache, snapshots, aliases):
    ...
    metadata = {
        'generatedAt': max(v for v in source_timestamps if v),
        'schemaVersion': 3,
        'primarySource': PRIMARY_SOURCE,
        'historicalUse': (
            'Historical divisional rankings are a data/research artifact. No '
            'runtime model code consumes them: v1 scoring and the frozen '
            '16-feature MODEL_V2 are unaffected by this module.'
        ),
        'kaggle': {
    ...
    parts = [
        ...
        '// Kaggle history is CC0; current snapshots come from UFC.com.',
        '//',
        '// DIVISION_RANK_HISTORY is a data/research artifact with no runtime',
        '// model consumer. Do not wire it into v1 scoring or MODEL_V2.',
        '',
        f'export const RANKINGS_METADATA = {js_json(metadata, pretty=True)};',
        '',
...
def main():
    ...
    generated_changed, metadata = write_generated(cache, snapshots, aliases)
    print(
        f"Kaggle v{metadata['kaggle']['version']} through "
        f"{metadata['kaggle']['historyUsedThrough']} (cached); official media "
        f"through {metadata['officialUfc']['mediaSnapshot']}"
    )
    print(
        f"Generated {metadata['history']['fighterDivisionSeries']} series, "
        f"{metadata['history']['transitions']} transitions, "
        f"{metadata['history']['explicitUnrankedTombstones']} removals"
    )
    print(
        'Changed: ' + ', '.join(changed_snapshots + (
            [str(OUTPUT_PATH.relative_to(ROOT))] if generated_changed else []
        )) if changed_snapshots or generated_changed else 'No ranking changes'
    )
