...
A **field replacement** is one changed JSON field. Because every one of them is a
one-line `"key": "value"`, a replacement shows up as a `-`/`+` pair, so
**diff lines = 2 × field replacements**. The two units are stated separately
below because an earlier revision of this document reported "32" and "4" without
saying which unit was meant, and neither figure matched either unit.

Recomputed at PR head `b2b5024` with all four pinned feeds:

| # | Comparison (direction is significant) | Field replacements | Diff lines |
|---|---|---:|---:|
| A | baseline `1adfd22` **committed** manifest → regenerated on a **pristine** `1adfd22` tree | **13** | **26** |
| B | **regenerated** baseline → PR head `b2b5024` committed manifest | **5** | **10** |
| C | baseline `1adfd22` **committed** manifest → PR head `b2b5024` committed manifest | **14** | **28** |

A is pre-existing drift: the baseline's committed manifest already disagreed
with its own artifacts before this branch existed. B is this branch's actual
effect. C is what a reviewer sees in the PR diff, and it matches `git diff
--numstat`'s `14 14` for the file.

The five fields in comparison **B**, named individually:

| Field | Change | Substantive to 6A? |
|---|---|---|
| `manifestGeneratedAt` | run timestamp | no — moves on every run |
| `generatedAt` | `2026-08-13` → `2026-08-15` | no — regeneration date |
| `contentHash` (`fightHistory`) | `f5dda9d4…be59` → `420eafc4…c449` | **yes — the one real 6A line** |
| `generatorVersion` (`fightHistory`) | `8cbd27d5…` → `47f7a74d…` | no — last-commit SHA of `update_fighters.py` |
| `generatorVersion` (`fightersDataAggregates`) | `8cbd27d5…` → `47f7a74d…` | no — same, second occurrence |

Reproduce all three from scratch:

```bash
# A — regenerate on a pristine baseline tree
git worktree add --detach /tmp/mbase 1adfd22fe3b09b84ddd994a7057d4cc9c0275276
cp ufc_*.csv /tmp/mbase/          # ufc_event_details.csv is gitignored; the script aborts without it
cp /tmp/mbase/src/sourceManifest.js /tmp/m_base_committed.js
cd /tmp/mbase && python3 generate_source_manifest.py
git diff --numstat -- src/sourceManifest.js          # -> 13  13
cp src/sourceManifest.js /tmp/m_base_regen.js

# B and C — against the PR head's committed manifest
git show b2b50242acdc96dd79b84c24938461394c019ac3:src/sourceManifest.js > /tmp/m_pr.js
diff /tmp/m_base_regen.js     /tmp/m_pr.js | grep -c '^< '   # B field replacements -> 5
diff -u /tmp/m_base_regen.js  /tmp/m_pr.js | grep -c '^[-+][^-+]'   # B diff lines   -> 10
diff /tmp/m_base_committed.js /tmp/m_pr.js | grep -c '^< '   # C field replacements -> 14
diff -u /tmp/m_base_committed.js /tmp/m_pr.js | grep -c '^[-+][^-+]'  # C diff lines -> 28
```

Only `manifestGeneratedAt` varies with *when* you run this, so A and C reproduce
exactly and B's five fields differ only in that one timestamp's value.
...
| Saved UFC 330 matchups (pre-event roster), both slot orders | 10 | **exactly equal** |
...
## UFC 330 — two distinct populations

These were previously conflated under one heading, which overstated the
coverage. They are separate populations answering separate questions.

| | A · pre-event roster | B · contested event |
|---|---|---|
| What it is | the 20 fighters in the 10 matchups saved to `src/upcomingData.js` before the card | UFC 330: Makhachev vs. Machado Garry as actually contested |
| Size | 10 matchups · 20 fighters | **12 bouts · 24 fighters** |
| Feed | pinned Greco `18ba2092`, which **predates the event** | later Greco `2feb5257` (2026-08-16 refresh) |
| What is asserted | historical `wc`/`tb` deltas of **59 / 7** on bouts these fighters had *before* UFC 330 | each bout's raw label → parsed division / championship / interim / tournament-final, and both emitted corners |
| Held by | `Ufc330PreEventRosterCoverage` | `Ufc330ContestedEventFixture` |

**The pinned feed contains no UFC 330 rows at all.** Population A therefore
verifies *pre-event* corrections and must not be described as "UFC 330's bouts
verified"; `test_pinned_feed_contains_no_ufc_330_rows` asserts that split
explicitly so a future re-pin cannot blur it.

Population B is covered by `tests/fixtures/weightclass/ufc330_bouts.tsv`, the 12
source rows captured verbatim so CI exercises the real event with **no network
fetch**. Provenance is recorded in the fixture header:

| Source | Value |
|---|---|
| Greco commit | `2feb525741e35472c5ef4673533eee7bc3e7cce8` |
| `ufc_fight_results.csv` | `e080cefa88ef03d5a8e76948e79a61ee093d30c941d2647961c1b37bde609b2b` |
| `ufc_event_details.csv` | `f4e4e89d9869e9383fb66ac547b5581e64c98f1806a509f6bc971158234a8456` |
| `ufc_fight_details.csv` | `94edf07c9102d090ca44fc560a19551137dcf806b99004a7404299225805bd7c` |
| `ufc_fight_stats.csv` | `124ce82e3c0a93f6347e08f80bfbcc5346c93a11bbfc6c5eef012f2fdfc9e486` |

The two bouts outside population A — **Charles Johnson vs. Eduardo Chapolin**
(`Catch Weight Bout` → Catch Weight) and **Rafael Tobias vs. Lucas Fernando**
(`Light Heavyweight Bout` → Light Heavyweight) — are exactly the gap the
20-fighter population cannot reach, and each is asserted by name. An isolated
current-feed dry run confirmed all 12 bouts: 24 fighters all present, both
corners identical on every bout, and the only two championships the
`UFC Welterweight Title Bout` and `UFC Women's Strawweight Title Bout` main and
co-main. That run was **evidence only** — no regenerated production artifact is
committed here, and a production refresh remains separately gated.
